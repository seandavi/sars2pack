...
abstract: |
link-citations: true  
  Open data resources tracking the nearly real-time global spread of COVID-19 pandemic have become the cornerstone of local, national, and international interventions to mitigate public health impacts. Here, we present sars2pack, a principled, open development, open source software package for accessing high-value COVID-19 related data sources in familiar tidy data form from within the R statistical environment. We foresee continued importance for fit-for-use COVID-19 data resources enhance data-driven decisions across multiple domains. 
...
more quickly and with a much higher death toll and the current
...
We used a combination of informal literature searches, attributed sources from media reports, popular online portals, and crowd-sourced data collections to find candidates for inclusion as a data resource in sars2pack. Additionally, we required each candidate dataset be available online machine readable format, be open access, and have an adequate description of methods of collection (evaluated subjectively by the authors). Datasets were prio collected openly available data resources for inclusion in the XXXXXXXX
