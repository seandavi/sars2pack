...
obtain estimates of R0 under exponential growth.  There
seems to be a data problem, and with the given
generation time, the TD and ML models throw errors.

```{r lkdfra, fig.height=7}
...
est <- estimate.R(epid=diff(itdat), GT=mGT, 
    methods=c("EG"), begin=1L, end=length(itdat)-1L)
par(mfrow=c(2,2))
est
plot2(est)
```

```{r lkdfra2, fig.height=7}
par(mfrow=c(2,2), mar=c(4,3,2,2))
plotfit2(est)
```
